...
import {
  EMOJI_INVADERS_UPGRADES,
  canPurchase,
  type UpgradeDef,
} from '@/lib/emoji-invaders-upgrades';

interface EmojiInvadersPlayerProps {
  gameId: string;
  initialLives: number;
...
export default function EmojiInvadersPlayer({
  gameId,
  initialLives,
  initialUpgrades,
}: EmojiInvadersPlayerProps) {
  ...
  const [purchasingUpgrade, setPurchasingUpgrade] = useState<string | null>(
    null
  );
  ...
  const categoryUpgrades = EMOJI_INVADERS_UPGRADES.filter(
          ...
          <div className="space-y-2 max-h-[50vh] overflow-y-auto">
            {categoryUpgrades.map((upgrade) => (
              <UpgradeCard
                ...
                canBuy={canPurchase(upgrade.id, ownedUpgrades)}
                canAfford={coinBalance >= upgrade.cost}
                purchasing={purchasingUpgrade === upgrade.id}
                locked={
                  !!upgrade.requires && !ownedUpgrades.has(upgrade.requires)
                }
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-900">
        <div className="flex items-center gap-3">
          <p className="text-white font-bold text-sm">Emoji Invaders</p>
      ...
      <iframe
        ref={iframeRef}
        src="/arcade/emoji-invaders/index.html"
        ...
        <div className="flex-1 min-w-0">
          ...
          {locked && upgrade.requires && (
            <p className="text-xs text-orange-500 font-semibold mt-1">
              Requires:{' '}
              {EMOJI_INVADERS_UPGRADES.find((u) => u.id === upgrade.requires)
                ?.name ?? upgrade.requires}
            </p>
          )}
        </div>
        {!owned && (
