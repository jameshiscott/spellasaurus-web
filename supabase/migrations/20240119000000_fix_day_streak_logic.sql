-- =============================================================================
-- Fix day streak logic in complete_spelling_session
-- Bug: streak was incrementing on every session, not once per unique day.
-- Fix: only increment when last_practised_at was yesterday, keep same if today.
-- =============================================================================
...
BEGIN
  ...
  --    Day streak logic:
  --      Same day as last practice   → streak stays the same
  --      Yesterday                   → increment streak by 1
  --      More than 1 day gap (or no previous) → reset to 1
  ...
  ON CONFLICT (child_id) DO UPDATE SET
    ...
    current_streak = CASE
      ...
      WHEN child_stats.last_practised_at::DATE = NOW()::DATE
        THEN child_stats.current_streak
